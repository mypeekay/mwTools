package com.peekay.mw_provider_demo;
...
import com.peekay.mw_provider_demo.bean.MintMark;
import com.peekay.mw_provider_demo.bean.MonsterBean;
import com.peekay.mw_provider_demo.bean.MovesInfo;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    public static final String TAG = "mw_manual_tag";
    private TextView tv_info;
    private Button btn_query_monster, btn_query_move, btn_query_mintmark, open_mw, test;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        Objects.requireNonNull(getSupportActionBar()).setTitle("MW手册Provider Demo程序");
        initView();
    }

    private void initView() {
        tv_info = findViewById(R.id.info);
        btn_query_monster = findViewById(R.id.btn_query_monster);
        btn_query_monster.setOnClickListener(this);
        btn_query_move = findViewById(R.id.btn_query_move);
        btn_query_move.setOnClickListener(this);
        btn_query_mintmark = findViewById(R.id.btn_query_mintmark);
        btn_query_mintmark.setOnClickListener(this);
        open_mw = findViewById(R.id.open_mw);
        open_mw.setOnClickListener(this);
    ...
    public void onClick(View v) {
        StringBuilder sb = new StringBuilder("若获取不到数据请先打开MW手册\n\n\n");
        switch (v.getId()) {
            case R.id.test:
                Toast.makeText(this, "测试按钮", Toast.LENGTH_SHORT).show();
                break;
            case R.id.open_mw:
//                String packageName = "com.peekay.mw_manual";//正式版手册应该是是这个，就注释下面那句，把这句取消注释
                String packageName = "com.peekay.mw_manual.debug";
                Context pkgContext = getPackageContext(this, packageName);
                Intent intent = getAppOpenIntentByPackageName(this, packageName);
                if (pkgContext != null && intent != null) {
                    pkgContext.startActivity(intent);
                }
                break;
            case R.id.btn_query_monster:
                //查询id为5000的精灵信息
                MonsterBean monsterBean = queryMonster("ID = ?", new String[]{"5000"});
                if (monsterBean != null) {
                    sb.append(monsterBean);
                }
                //查询精灵名称为启灵元神的精灵信息
                monsterBean = queryMonster("DefName = ?", new String[]{"启灵元神"});
                if (monsterBean != null) {
                    sb.append("\n\n\n").append(monsterBean);
                }
                List<MonsterBean> monsterBeans = queryMonsters("defname like ? and typeid = ?", new String[]{"%尔%", "1"});
                sb.append("\n\n\n精灵名称包含 尔 且 typeid=1 的精灵有")
                        .append(monsterBeans.size()).append("条信息");
                break;
            case R.id.btn_query_move:
                List<MovesInfo> movesInfos = queryMoves("name = ?", new String[]{"跪地求饶"});
//                List<MovesInfo> movesInfos = queryMoves("name like ?", new String[]{"%偷袭%"});
                sb.append(new Gson().toJson(movesInfos));//转为JSON格式
//                sb.append(movesInfos);
                break;
            case R.id.btn_query_mintmark:
                List<MintMark> mintMarks = queryMintmarks("des like \"%十年%\"", null);
//                List<MintMark> mintMarks = queryMintmarks("des like ?", new  String[]{"%k13%"});
                sb.append(mintMarks);
//                sb.append(mintMarks.get(0)).append("\n\n").append(mintMarks.get(1));
                break;
        }
